        ...
        if(NULL!=mTerrainGroup)
        {
            if(READY!=mLoadingState)
            {
                ...
                while(it.hasMoreElements())
                {
                    ...
                    {
                        Ogre::Root::getSingleton().getWorkQueue()->removeResponseHandler(terrain->WORKQUEUE_DERIVED_DATA_REQUEST,terrain);
                        Ogre::Root::getSingleton().getWorkQueue()->abortRequestsByChannel(terrain->WORKQUEUE_DERIVED_DATA_REQUEST);
                    }
                }
            }
    ...
    Json::Value TerrainManager::toJson()
    {
        ...
        while(it.hasMoreElements())
        {
            ...
            if(NULL==terrain)
            {
                ...
                Debug::error("TerrainManager::toJson(): slot ")(slot->x)("x")(slot->y)(" has no terrain defined ! Skipping slot.").endl();
                continue;
            }
            Json::Value terrainValue;
            terrainValue["slotPosition"]=StringUtils::toJson(Ogre::Vector2(static_cast<float>(slot->x),static_cast<float>(slot->y)));
