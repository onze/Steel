...
namespace Steel
{
    ...
    SelectionManager::SelectionManager(Engine *engine):mSelection(Selection())
    {
        mEngine=engine;
    }

    SelectionManager::SelectionManager(const SelectionManager& o):mSelection(o.mSelection)
    {
    ...
    Ogre::Quaternion SelectionManager::selectionOrientationFromCenter()
    {
        ...
        if (agent == NULL)
        {
            Debug::error("SelectionManager::selectionOrientationFromCenter(): selection's first item (agent ")(aid)(") is not valid.").endl();
            return Ogre::Quaternion::IDENTITY;
        }
