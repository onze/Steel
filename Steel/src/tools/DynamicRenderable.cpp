...
namespace Steel
{
    ...
    void DynamicRenderable::prepareHardwareBuffers(size_t vertexCount, size_t indexCount)
    {
        ...
        if(newVertCapacity != mVertexBufferCapacity)
        {
            ...
            HardwareVertexBufferSharedPtr vbuf = HBM.createVertexBuffer(mRenderOp.vertexData->vertexDeclaration->getVertexSize(0),
        ...
        if(mRenderOp.useIndexes)
        {
            ...
            if(newIndexCapacity != mIndexBufferCapacity)
            {
                ...
                mRenderOp.indexData->indexBuffer = HBM.createIndexBuffer(HardwareIndexBuffer::IT_16BIT,
